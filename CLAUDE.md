# bunnie-workflows 개발 규칙

## 스킬 유형

- **진화 스킬**: 실전에서 발견된 패턴을 strategy 파일에 축적. core/evolution-engine.md의 6단계 프로세스를 따른다. 현재 테스트 도메인(test-evolve)에서만 검증됨.
...
`global-rules.md`가 설치 시 `~/.claude/CLAUDE.md`로 심볼릭 링크된다. 글로벌 규칙을 수정할 때는 `global-rules.md`를 편집.
...
bunnie-workflows/
  ...
  strategy/                # 도메인별 축적된 전략
    testing/perspectives.md
  skills/                  # 스킬 (진화 + 정적)
  templates/               # 프로젝트/스킬/전략 템플릿
  scripts/                 # 개발 도구
    validate-structure.sh  # 구조 검증 스크립트
  global-rules.md          # 설치 시 ~/.claude/CLAUDE.md로 링크
```
