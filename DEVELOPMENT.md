...
- **진화 스킬**: 실전에서 발견된 패턴을 strategy 파일에 축적. core/evolution-engine.md의 6단계 프로세스(DETECT→ANALYZE→EXTRACT→APPLY→RECORD→PROPAGATE)를 따른다. 현재 테스트 도메인(test-evolve)에서만 검증됨.
